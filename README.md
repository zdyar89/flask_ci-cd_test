...
- 🎯 Entry points to execute your program using `python -m <flask_ci_cd_test>` or `$ flask_ci_cd_test` with basic CLI argument parsing.
...
---
# flask_ci_cd_test Flask Application

Awesome flask_ci_cd_test created by zdyar89
...
```bash
git clone https://github.com/zdyar89/flask_ci-cd_test flask_ci_cd_test
cd flask_ci_cd_test
make install
```
...
```bash
flask_ci_cd_test create-db   # run once
flask_ci_cd_test populate-db  # run once (optional)
flask_ci_cd_test add-user -u admin -p 1234  # ads a user
flask_ci_cd_test run
```
