...
original_author="zdyar89"
original_name="flask_ci_cd_test"
original_urlname="flask_ci-cd_test"
original_description="Awesome flask_ci_cd_test created by zdyar89" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flask_ci_cd_test "${name}"
...
then
    ...
    echo "then run 'flask_ci_cd_test' or 'python -m flask_ci_cd_test'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flask_ci_cd_test' or 'python -m flask_ci_cd_test' or 'poetry run flask_ci_cd_test'"
fi
